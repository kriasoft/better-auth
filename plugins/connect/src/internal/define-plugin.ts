 ...
 * export const connectPlugin = () => createComplexPlugin() // ❌ Deep types exposed
 *
 * // After: Controlled boundary prevents depth issues
 * export const connectPlugin = () => definePlugin<ConnectEndpoints>(createComplexPlugin()) // ✅
 * ```
 */
 ...
 *   plugins: withPlugins(organization(), connectPlugin()) // Type-safe
 * });
 * ```
 */
